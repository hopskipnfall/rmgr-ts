...
const CHARACTER_NAMES: Record<number, string> = {
  ...
  0x00: "Mario", 0x01: "Fox", 0x02: "Donkey Kong", 0x03: "Samus",
  0x04: "Luigi", 0x05: "Link", 0x06: "Yoshi", 0x07: "Captain Falcon",
  0x08: "Kirby", 0x09: "Pikachu", 0x0a: "Jigglypuff", 0x0b: "Ness",
  0x0c: "Master Hand", 0x0d: "Metal Mario",
  0x0e: "Polygon Mario", 0x0f: "Polygon Fox", 0x10: "Polygon DK",
  0x11: "Polygon Samus", 0x12: "Polygon Luigi", 0x13: "Polygon Link",
  0x14: "Polygon Yoshi", 0x15: "Polygon Falcon", 0x16: "Polygon Kirby",
  0x17: "Polygon Pikachu", 0x18: "Polygon Jigglypuff", 0x19: "Polygon Ness",
  0x1a: "Giant DK", 0x1b: "Random",
  // Remix fighters
  0x1d: "Falco", 0x1e: "Ganondorf", 0x1f: "Young Link", 0x20: "Dr. Mario",
  0x21: "Wario", 0x22: "Dark Samus", 0x23: "Link (EU)", 0x24: "Samus (JP)",
  0x25: "Ness (JP)", 0x26: "Lucas", 0x27: "Link (JP)", 0x28: "Falcon (JP)",
  0x29: "Fox (JP)", 0x2a: "Mario (JP)", 0x2b: "Luigi (JP)", 0x2c: "DK (JP)",
  0x2d: "Pikachu (EU)", 0x2e: "Jigglypuff (JP)", 0x2f: "Jigglypuff (EU)",
  0x30: "Kirby (JP)", 0x31: "Yoshi (JP)", 0x32: "Pikachu (JP)",
  0x33: "Samus (EU)", 0x34: "Bowser", 0x35: "Giga Bowser", 0x36: "Piano",
  0x37: "Wolf", 0x38: "Conker", 0x39: "Mewtwo", 0x3a: "Marth", 0x3b: "Sonic",
  0x3c: "Sandbag", 0x3d: "Super Sonic", 0x3e: "Sheik", 0x3f: "Marina",
  0x40: "King Dedede", 0x41: "Goemon", 0x42: "Peppy", 0x43: "Slippy",
  0x44: "Banjo", 0x45: "Metal Luigi", 0x46: "Ebisumaru", 0x47: "Dragon King",
  0x48: "Crash", 0x49: "Peach", 0x4a: "Roy", 0x4b: "Dr. Luigi",
  0x4c: "Lanky Kong",
  // Remix polygons
  0x4d: "Polygon Wario", 0x4e: "Polygon Lucas", 0x4f: "Polygon Bowser",
  0x50: "Polygon Wolf", 0x51: "Polygon Dr. Mario", 0x52: "Polygon Sonic",
  0x53: "Polygon Sheik", 0x54: "Polygon Marina", 0x55: "Polygon Falco",
  0x56: "Polygon Ganondorf", 0x57: "Polygon Dark Samus", 0x58: "Polygon Marth",
  0x59: "Polygon Mewtwo", 0x5a: "Polygon Dedede", 0x5b: "Polygon Young Link",
  0x5c: "Polygon Goemon", 0x5d: "Polygon Conker", 0x5e: "Polygon Banjo",
  0x5f: "Polygon Peach", 0x60: "Polygon Crash",
};
...
const STAGE_NAMES: Record<number, string> = {
  0x00: "Peach's Castle", 0x01: "Sector Z", 0x02: "Congo Jungle",
...
function gameTypeName(gameType: number): string {
  return { 1: "Time", 2: "Stock", 3: "Time + Stock" }[gameType] ?? `unknown (${gameType})`;
}
...
function determineWinner(replay: Replay): string {
  ...
  if (!replay.gameEnd) {
    ...
    const best = seatedPorts
      .map((port) => [port, lastFrame?.ports[port]?.post.stocksRemaining ?? -Infinity] as const)
      .sort((a, b) => b[1] - a[1])[0];
...
function inspectFile(path: string): void {
  ...
  console.log(`\n=== ${path} ===`);
  console.log(`Format version: ${replay.header.version} | Complete: ${replay.isComplete}`);
  console.log(`Stage: ${stageName(gameStart.stageId)} | Mode: ${gameTypeName(gameStart.gameType)}`);
  console.log(
  ...
  console.log(`Frames recorded: ${replay.frames.length} (~${durationSeconds.toFixed(1)}s @ 60fps)`);
  console.log(`Teams: ${gameStart.teamsEnabled ? "on" : "off"} | Handicap: ${gameStart.handicapMode}`);

  console.log("Players:");
  for (const port of getSeatedPorts(replay)) {
    const settings = gameStart.ports[port];
    const finalStocks = gameEnd ? humanStocks(gameEnd.placements[port], true) : "unknown (truncated recording)";
    const extras: string[] = [];
    if (gameStart.teamsEnabled) extras.push(`team ${settings.team}`);
    if (gameStart.handicapMode !== "off") extras.push(`handicap ${settings.handicap}`);
    if (settings.slotType === "cpu") extras.push(`CPU level ${settings.cpuLevel}`);
    console.log(
  ...
  console.log(`Result: ${gameEnd ? gameEnd.endReason : "unknown (truncated recording)"}`);
  console.log(`Winner: ${determineWinner(replay)}`);
}

const files = process.argv.slice(2);
if (files.length === 0) {
  console.error("Usage: node examples/inspect-replay.ts <file.rmgr> [more files...]");
  process.exit(1);
}
